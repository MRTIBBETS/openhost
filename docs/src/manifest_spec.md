...
Declares that the app implements a federation protocol, identified by the URL of its spec document. When the owner opens a federation invite, the zone's `/federation/connect` page finds installed apps whose `url` matches the invite's `spec` parameter and offers a click-through into each app's own connect page (at `connect_path`, with the invite's full query string passed along).
