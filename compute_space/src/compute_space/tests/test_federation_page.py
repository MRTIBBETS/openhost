...
def test_lists_matching_app_with_passthrough_query(cfg: Any) -> None:
    ...
    with TestClient(app=_build_app(cfg)) as client:
        resp = client.get("/federation/connect", params=INVITE_PARAMS, cookies=cookie)
...
def test_missing_spec_is_rejected(cfg: Any) -> None:
    ...
    with TestClient(app=_build_app(cfg)) as client:
        resp = client.get("/federation/connect", params={"source": "https://a.example.com"}, cookies=cookie)
...
def test_unauthenticated_redirects_to_login(cfg: Any) -> None:
    ...
    with TestClient(app=_build_app(cfg)) as client:
        resp = client.get("/federation/connect", params=INVITE_PARAMS, follow_redirects=False)
...
def test_non_http_spec_is_not_linkified(cfg: Any) -> None:
    ...
    with TestClient(app=_build_app(cfg)) as client:
        resp = client.get("/federation/connect", params=params, cookies=cookie)
    assert resp.status_code == 200
